...
void HyperCube_Class::HyperCube_QuickSort() {
	...
	if (nbr_k > my_id) {
	    ...
	    // ***** Add MPI call here *****
	    MPI_Send(&list_size_gt,1,MPI_INT,nbr_k,1,sub_hypercube_comm);


	    // MPI-3: Receive number of elements less than or equal to pivot

	    // ***** Add MPI call here *****
	    MPI_Recv(&list_size_leq,1,MPI_INT,nbr_k,2,sub_hypercube_comm,&status);
...
printf("%0d \n",list_size_leq);
printf("size less \n");
	    MPI_Send(&list[idx],list_size_gt,MPI_INT,nbr_k,3,sub_hypercube_comm);
	   
	    // MPI-5: Receive neighbor's list of elements that are less than or equal to pivot
            
	    int *temp2 = new int[list_size_leq];
		...
		*(temp2+i) = *(list+i);
	    ...
	    MPI_Recv(list,list_size_leq,MPI_INT,nbr_k,4,sub_hypercube_comm,&status);
	    ...
	    new_list = merged_list(list, idx, nbr_list, nbr_list_size); 
            for(int i=0; i<idx+nbr_list_size; i++)
            {
	//	printf("XXXXXXXXXXXXXXXXXX\n");
	    ...
	    // ***** Add MPI call here *****
            ...
            MPI_Recv(&list_size_gt,1,MPI_INT,nbr_k,1,sub_hypercube_comm,&status);
	    // MPI-7: Send number of elements less than or equal to pivot

	    // ***** Add MPI call here *****
 	    MPI_Send(&list_size_leq,1,MPI_INT,nbr_k,2,sub_hypercube_comm);
	    ...
	    // ***** Add MPI call here *****
	    MPI_Recv(&list[idx],list_size_gt,MPI_INT,nbr_k,3,sub_hypercube_comm,&status);
	    ...
	    // ***** Add MPI call here *****
	    MPI_Send(list,list_size_leq,MPI_INT,nbr_k,4,sub_hypercube_comm);
